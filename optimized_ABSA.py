...
logging.basicConfig(filename="script_progress.log", level=logging.INFO, 
...
def process_reviews_chunk(reviews_chunk, batch_size=32):
    ...
    for idx, entry in reviews_chunk.iterrows():
        try:
            ...
            logging.error(f"Error processing review {entry['asin']} - {entry['user_id']}: {e}")

    return filtered_reviews

def perform_absa_and_save(data, output_path, chunk_size=1000, batch_size=32, max_workers=10):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_chunk = {
            executor.submit(process_reviews_chunk, data[i:i + chunk_size], batch_size): i
        ...
        for future in as_completed(future_to_chunk):
            try:
                ...
                print(result)
                save_json(filtered_reviews, output_path)  # Save intermediate results to reduce memory usage
                logging.info(f"Processed chunk: {len(filtered_reviews)} reviews saved so far.")
