...
df_metaData_raw_cellPhones['brand'] = df_metaData_raw_cellPhones['details'].apply(extract_brand)
...
pprint.pprint(Cell_Phones_df_raw.head())
# Useful Cols
subset_cols = ['category_0', 'category_1', 'category_2', 'category_3', 'description', 'title', 'bought_together', 'images', 'features', 'details', 'main_category', 'price', 'num_ratings', 'avg_rating', 'asin', 'parent_asin', 'brand']
...
if not os.path.exists('./metaData_for_cellPhones.pkl'):
    ...
    df_asin_numReviews['asin'] = reviews_for_cellPhones.groupby(by="asin").count()[['text']].index
    df_asin_numReviews['num_reviews'] = reviews_for_cellPhones.groupby(by="asin").count()[['text']].text.values
...
if not os.path.exists("./reviews_wholeReview.txt"):
    ...
    wholeReview_reviews_for_cellPhones_df['Index'] = reviews_for_cellPhones_df_cleaned[['asin', 'user_id']].apply(lambda x: '_'.join(x), axis=1)
    wholeReview_reviews_for_cellPhones_df['text'] = reviews_for_cellPhones_df_cleaned.text.values
    wholeReview_reviews_for_cellPhones_df = wholeReview_reviews_for_cellPhones_df.dropna(axis=0, subset=['text'])
